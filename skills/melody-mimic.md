...
**重要**: 在进入创作模式之前，必须生成以下两份分析报告：

##### 报告 1: 旋律特征分析报告 🎵

基于 MIDI 分析生成，包含：
- 基本信息（总音符数、音域、Ticks per beat）
- 调式分析（检测结果、主音、置信度、情感特征）
- 节奏型分布（各类节奏占比的可视化柱状图）
- 音程分布（大跳、小跳、同度、级进占比）
- 旋律方向（上行、下行、平稳占比）
- 旋律轮廓（拱形、谷形、平稳、连续上行、连续下行占比）

##### 报告 2: 歌词深度分析报告 📝

基于歌词文件生成，包含：
- 基本信息（歌曲名称、演唱者、总行数、总字数、结构）
- 整体结构分析（段落划分、字数结构）
- 主歌部分分析（原文、内容层次、意象分析、修辞手法、节奏特点）
- 副歌部分分析（原文、内容层次、意象分析、修辞手法、节奏特点）
- 整体艺术特色（风格元素、情感表达、节奏与韵律、意象特点）
- 词曲契合度分析（歌词特点与旋律特点的对应关系）
- 创作启示（对新歌创作的具体指导）
- 数据统计（词频统计、句式统计）
- 核心特征总结
...
| 报告 | 主要作用 | 指导内容 |
|------|----------|----------|
| 旋律特征报告 | 提供音乐技术参数 | 调式选择、节奏型、音程跳跃、旋律走向 |
| 歌词深度报告 | 提供文学创作参数 | 意象选择、句式结构、情感表达、修辞手法 |

**双报告缺一不可**：
- ❌ 只有旋律报告 → 新歌词可能与旋律风格不匹配
- ❌ 只有歌词报告 → 新旋律可能与原曲风格偏离
- ✅ 双报告齐全 → 词曲风格高度一致
...
- `[Verse 1]`, `[Verse 2]`: 主歌段落（推荐首字母大写+空格，兼容 Suno/Tunee）
- `[Chorus]`: 副歌段落
- `[Bridge]`: 桥段（可选）
- `[Outro]`: 尾声（可选）
- 每行一句歌词，空行分隔段落内的句子组
...
**格式兼容性**:
- ✅ 推荐格式: `[Verse 1]`, `[Chorus]` (首字母大写+空格) - Suno/Tunee 标准
- ⚠️ 兼容格式: `[verse1]`, `[chorus]` (小写) - 可识别但不标准
- ✅ 支持中英文: `[主歌1]`, `[副歌]` - 仅用于内部分析
...
|----------|----------|----------|------|
| `[info]` | - | `[信息]` | 歌曲元信息段落 |
| `[Verse 1]`, `[Verse 2]` | `[verse1]`, `[verse2]` | `[主歌1]`, `[主歌2]` | 主歌段落 |
| `[Chorus]` | `[chorus]` | `[副歌]` | 副歌段落 |
| `[Bridge]` | `[bridge]` | `[桥段]` | 桥段 |
| `[Pre-Chorus]` | `[pre-chorus]` | `[预副歌]` | 预副歌段落 |
| `[Outro]` | `[outro]` | `[尾声]` | 尾声段落 |
| `[Intro]` | `[intro]` | `[前奏]` | 前奏段落（通常无歌词） |

**识别规则**:
- 段落标记必须独占一行
- **推荐格式**: `[Verse 1]`, `[Chorus]` (首字母大写+空格) - 符合 Suno/Tunee/Udio 标准
- **兼容格式**: `[verse1]`, `[chorus]` (小写) - 可识别但不推荐用于导出
- 标记不区分大小写: `[Verse1]` 等同于 `[verse1]` (内部解析)
- 支持中英文标记混用（中文仅用于内部分析）

**正则表达式**:
```
段落标记: /^\[(info|verse\s*\d*|chorus\s*\d*|bridge|pre-chorus|outro|intro|信息|主歌\d*|副歌|桥段|预副歌|尾声|前奏)\]$/i
```
**说明**: 支持 `[Verse 1]` (带空格) 和 `[verse1]` (不带空格) 两种格式
...
调式推断基于音阶特征音分析，用于识别参考歌曲的调式类型。系统支持多种音乐风格的调式检测。
...
| 调式 | 英文名 | 特征音级 | 情感特征 |
|------|--------|----------|----------|
| 大调 | Major (Ionian) | 0, 2, 4, 5, 7, 9, 11 | 明亮、欢快、积极 |
| 小调 | Minor (Aeolian) | 0, 2, 3, 5, 7, 8, 10 | 忧伤、深沉、内敛 |
| 多利亚 | Dorian | 0, 2, 3, 5, 7, 9, 10 | 爵士、蓝调、神秘 |
| 混合利底亚 | Mixolydian | 0, 2, 4, 5, 7, 9, 10 | 摇滚、乡村、放松 |
| 弗里几亚 | Phrygian | 0, 1, 3, 5, 7, 8, 10 | 西班牙、弗拉门戈、异域 |
| 利底亚 | Lydian | 0, 2, 4, 6, 7, 9, 11 | 梦幻、飘逸、电影感 |

**中国五声调式**:

| 调式 | 中文名 | 主音 | 特征音级 | 情感特征 |
|------|--------|------|----------|----------|
| 宫调式 | Gong | 1 (Do) | 0, 2, 4, 7, 9 | 明亮、庄重、喜庆 |
| 商调式 | Shang | 2 (Re) | 0, 2, 4, 7, 9 | 深沉、内敛、怀旧 |
| 角调式 | Jue | 3 (Mi) | 0, 2, 4, 7, 9 | 清新、空灵、禅意 |
| 徵调式 | Zhi | 5 (Sol) | 0, 2, 4, 7, 9 | 热情、奔放、豪迈 |
| 羽调式 | Yu | 6 (La) | 0, 2, 4, 7, 9 | 忧伤、婉转、抒情 |

**其他音阶**:

| 音阶 | 英文名 | 特征音级 | 适用风格 |
|------|--------|----------|----------|
| 蓝调音阶 | Blues Scale | 0, 3, 5, 6, 7, 10 | 蓝调、爵士、摇滚 |
| 和声小调 | Harmonic Minor | 0, 2, 3, 5, 7, 8, 11 | 古典、金属、中东 |
| 日本音阶 | Japanese Scale | 0, 1, 5, 7, 8 | 日本传统、东方 |
| 阿拉伯音阶 | Arabic Scale | 0, 1, 4, 5, 7, 8, 11 | 中东、阿拉伯 |

**自动检测逻辑**:
1. 首先检测是否为五声音阶（音符集中在 5 个音级）
2. 如果是五声音阶，进一步判断中国五声调式
3. 如果不是，检测西方七声调式
4. 根据特征音和结束音确定具体调式
...
**重要**: 生成的简谱中**不要包含**版权提醒、免责声明等说明性文字，只输出纯净的简谱内容。
...
#### 6. 简谱生成注意事项 ⚠️
**关键规则**：生成简谱时必须遵守以下规则：
- ✅ **只输出纯净的简谱内容**：调号、拍号、BPM、段落标记、音符和歌词
- ❌ **严禁添加版权提醒文字**：不要在简谱中添加"版权提醒"、"⚠️"等文字
- ❌ **严禁添加免责声明**：不要在简谱中添加"免责声明"、"仅供学习参考"等段落
- ❌ **严禁添加说明性文字**：不要添加任何解释性、提示性的文字内容
- ✅ **保持格式简洁**：只包含音乐相关的符号和歌词文本

**错误示例**（不要这样做）：
```
| #1 #1 #2 #1' |
  版 权 提 醒 本  ← ❌ 错误：不要把提醒文字当作歌词
```

**正确示例**（应该这样做）：
```
| #1 #1 #2 #1' |
  故 人 音 创  ← ✅ 正确：只包含实际的歌词内容
```
...
【主歌A】 第{起始}-{结束}行   ({字数}字)
【副歌1】 第{起始}-{结束}行   ({字数}字)
【主歌B】 第{起始}-{结束}行   ({字数}字) - {备注}
【副歌2】 第{起始}-{结束}行   ({字数}字) - {备注}
...
```

### 字数结构

**主歌字数分布**: {每行字数，如：8-6-4-8-6-2-5-5-6-4} (共{总字数}字)
**副歌字数分布**: {每行字数，如：11-5-11-8-7-9} (共{总字数}字)
...
**风格类型**: {检测到的风格，如：国风/流行/摇滚/R&B/民谣/电子等}
...
**{风格关键词1}** + **{风格关键词2}** + **{风格关键词3}**

---

## 📌 结论

{总结性描述，说明歌曲如何将各种元素融合在一起，形成独特风格}
...
└── output/
    └── {song-name}/
        ├── {song-name}-旋律分析报告.txt    # 旋律特征分析报告
        ├── {song-name}-歌词分析报告.md     # 歌词深度分析报告
        └── ...                             # 后续生成的文件
```
...
1. **频繁引用报告数据**
   - "根据旋律分析报告，大跳音程占 50.8%，因此新歌词应保持意象跳跃..."
   - "根据歌词分析报告，主歌字数结构为 8-6-4-8-6-2-5-5-6-4，新歌词应遵循..."
...
3. **在确认节点展示对比**
   ```
   ✅ 字数结构检查: 8-6-4-8-6-2-5-5-6-4 (与原曲一致)
   ✅ 意象跳跃检查: 5个意象/段 (符合大跳音程特点)
   ✅ 情感基调检查: 清新婉转 (符合角调式特征)
   ```
...
*Claude 将自动检测您的环境，选择最佳分析模式，生成双报告，并提供专业级的音乐分析结果。*
